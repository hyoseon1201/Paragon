...
- 활성화된 플러그인: ModelingToolsEditorMode, ModelContextProtocol, Terminal, EditorToolset
...
**프로젝트 목표 (중요, 의사결정의 최우선 기준):** 약 2개월 내에 (1) 비주얼적으로 그럴듯한 완성도와 (2) 언리얼 멀티플레이어 네트워킹 역량을 어필할 수 있는 포트폴리오를 만드는 것이 목표입니다. 콘텐츠 볼륨(맵 크기, 라인/정글 수, 캐릭터 수 등)을 키우는 것보다 적은 콘텐츠를 더 다듬고, 서버 권위 로직/GAS 리플리케이션/예측 같은 네트워킹 코드의 완성도를 높이는 쪽으로 시간을 배분합니다. 작업 범위를 결정할 때 항상 이 목표를 기준으로 우선순위를 판단합니다.

**진행 단계:** 초기 프로토타입. 어빌리티, 전투, 매치메이킹 등 핵심 게임플레이 시스템을 처음부터 쌓아 올리는 단계이며, 아직 기능 완성이나 폴리싱/최적화 단계는 아닙니다.
...
| `AP1PlayerController` | 완료 | Enhanced Input 연결 (Move/Look/Jump), `AP1PlayerCameraManager` 지정 |

### Characters (`Source/P1/Characters/`)

| 클래스 | 상태 | 비고 |
|---|---|---|
| `AP1CharacterBase` | 완료 | Abstract 베이스, `IAbilitySystemInterface` + `IGenericTeamAgentInterface`, CachedASC 포인터, `IsSameTeam()` 헬퍼 |
| `AP1HeroCharacter` | 완료 | ASC `InitAbilityActorInfo` (서버: `PossessedBy` / 클라: `OnRep_PlayerState`), `DefaultAttributesEffect` 적용, `MovementSpeed` 어트리뷰트 → `CharacterMovementComponent` 바인딩 |
...
| 클래스 | 상태 | 비고 |
|---|---|---|
| `UP1AbilitySystemComponent` | 완료 | InputTag → Ability press/release 라우팅 구현 (`AbilityInputTagPressed` / `AbilityInputTagReleased`) |
| `UP1AttributeSet` | 완료 | 아래 어트리뷰트 전체 구현 — 리플리케이션, 클램핑 포함 |
| `UP1GameplayTags` (`P1GameplayTags.h/.cpp`) | 완료 | Native 태그: `InputTag.Ability.BasicAttack`, `Ability.BasicAttack`, `State.Attacking`, `Event.Montage.BasicAttackHit` |
| `UP1GameplayAbility` (베이스) | 완료 | `InstancedPerActor` + `LocalPredicted` 기본값, `GetP1Character/PlayerController` 헬퍼 |
| `UP1DamageGameplayAbility` | 완료 | 데미지 어빌리티 공통 베이스 — `DamageEffectClass` + `ApplyDamageToTarget()` 헬퍼 |
| `UP1GameplayAbility_MeleeAttack` | 완료 | 근접 기본공격 — 몽타주 재생 + `BasicAttackHit` 이벤트 → 구체 오버랩 → 데미지 적용 |
| GameplayEffect 데이터 에셋 | 미구현 | `DefaultAttributesEffect`, `GE_BasicAttackDamage` 등 BP 에셋 미생성 |
...
| Enhanced Input IMC / IA 에셋 | 미구현 | BP/에셋에서 `DefaultMappingContext`, `MoveAction`, `LookAction`, `JumpAction` 지정 필요 |

### 미구현 주요 시스템

- [ ] 기본공격 AnimNotify (`AN_BasicAttackHit`) 생성 및 몽타주 배치
- [ ] `UP1ExecCalc_Damage` + `GE_BasicAttackDamage` GameplayEffect 에셋
- [ ] 기본공격 입력 바인딩 (`PlayerController` → `ASC.AbilityInputTagPressed/Released`) + 어빌리티 그랜트
- [ ] Q/W/E/R 스킬 어빌리티
- [ ] `GameplayEffect` 데이터 에셋 (기본 스탯 초기화, 데미지, 힐, 버프/디버프)
- [ ] 데미지 계산 파이프라인 (`PostGameplayEffectExecute` 확장)
- [ ] 어빌리티 입력 바인딩 (`PlayerController` → `ASC.AbilityInputTagPressed/Released` 연결)
- [ ] 미니언 / AI 캐릭터 (`AP1MinionCharacter`, AIController)
- [ ] UI / HUD (체력바, 마나바, 스킬 쿨다운)
