...
- [ ] **상점/인벤토리 시스템** — 다음 작업. 설계 논의 중(인벤토리=PlayerState, 상점 카탈로그/구매 로직 위치는 결정 필요)
