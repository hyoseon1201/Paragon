...
// 역할 분담 — 실제 GAS 판정/커밋은 전부 어빌리티가 하고, 이 액터는 "배치된 원형 장판이 적(히어로+정글
// 몬스터)의 이동을 막는다"는 순수 공간적 판정만 담당). 데미지/디버프가 아니라 위치 자체를 매 틱 clamp하는
...
// 만으로는 표현 불가능하고, 결국 팀 판별은 항상 런타임에 동적으로 해야 한다(정글 몬스터는 TeamId=255라
// 별도 분기 없이도 항상 "적"으로 판별됨 — IsBlockableEnemy() 참고). 배치 시점에 이미 원 안에
// 있던 적은 못 나가게(감금), 밖에 있던 적은 못 들어오게(차단) — 각자 "시작한 쪽"을 벗어나지 못하게 매 틱 clamp한다.
//
...
public:
	...
	// 적(히어로+정글 몬스터)을 스캔해 감금 대상으로 등록한다.
